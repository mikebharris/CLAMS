...
var lambdaToBuild = flag.String("lambda", "all", "Which Lambda functions to test and/or build: <name-of-lambda> or all")
var stage = flag.String("stage", "", "Deployment stage: unit-test, build, int-test, init, plan, apply, destroy")

func main() {
	flag.Parse()
	var lambdas []string
	if shouldBuildAllLambdas() {
		lambdas = getListOfAllLambdas()
	} else {
		lambdas = []string{*lambdaToBuild}
	}
	switch *stage {
	case "unit-test":
		unitTest(lambdas)
...
func shouldBuildAllLambdas() bool {
	return *lambdaToBuild == "all"
}

func getListOfAllLambdas() []string {
	var lambdas []string
	entries, err := os.ReadDir("lambdas")
	if err != nil {
		log.Fatalf("error getting entries in lambdas directory: %s", err)
	}
	for _, entry := range entries {
		if _, err := os.Stat(fmt.Sprintf("lambdas/%s/Makefile", entry.Name())); err == nil {
			lambdas = append(lambdas, entry.Name())
		}
	}
	return lambdas
}
...
func integrationTest(lambdas []string) {
	for _, lambda := range lambdas {
